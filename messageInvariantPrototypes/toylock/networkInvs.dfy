...
module NetworkInvariants {
  ...
  // All msg have a valid ringPos as dst
  ghost predicate MsgValidDst(c: Constants, v: Variables)
    requires v.WF(c)
  {
    forall msg | msg in v.network.sentMsgs
    :: 0 <= msg.dst < |c.hostConstants|
  }
  ...
  // ghost predicate payloadLeqSenderMax(c: Constants, v: Variables) 
  //   requires v.WF(c)
  // {
  //   forall msg | msg in v.network.sentMsgs && 0 <= msg.src < |c.hostConstants|
  ...
  ghost predicate NetworkInv(c: Constants, v: Variables)
  {
    && v.WF(c)
