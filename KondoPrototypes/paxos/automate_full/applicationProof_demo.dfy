...
/// Generated 03/08/2024 21:20 Pacific Standard Time
  ...
  decreases c, v
{
 ...
 | v.ValidHistoryIdx(i) && c.ValidLearnerIdx(lnr) && v.History(i).learners[lnr].HasLearnedValue(val) :: 
    exists b: LeaderId
 :: 
    true && ChosenAtLearner(c, v.History(i),  VB(val, b), lnr)
}
  ...
  ensures LearnedImpliesQuorumOfAccepts(c, v')
{
      ...
      && ChosenAtLearner(c, v'.History(i), vb, lnr)
  {
    ...
    assert v'.ValidHistoryIdx(i);
    assert exists b: LeaderId ::
      && var vb := VB(val, b);
      && ChosenAtLearner(c, v'.History(i), vb, lnr);
  }
}
...
{
  VariableNextProperties(c, v, v');
  // assert LearnerReceivedAcceptImpliesAccepted(c, v'.History(i));
  assert LearnerReceivedAcceptImpliesAccepted(c, v);  // modified from above
}
...
// modified, need receive invaraint skolemization
...
lemma LeaderHearsDifferentValueFromChosenImpliesFalse(c: Constants, v: Variables, ldr: LeaderId, chosen: ValBal)
  requires v.WF(c)
  requires Chosen(c, v.Last(), chosen)  // modified from Chosen(c, v, chosen)
  requires c.ValidLeaderIdx(ldr)
  ...
  decreases c, v, ldr, chosen
{
  ...
  LeaderHearsDifferentValueFromChosenImpliesFalse(c, v, ldr', chosen);  // modified from LeaderHearsDifferentValueFromChosenImpliesFalse(c, v.Last(), ldr', chosen)
}
...
lemma LearnedImpliesChosen(c: Constants, v: Variables, lnr: LearnerId, val: Value)
  ...
  requires LearnedImpliesQuorumOfAccepts(c, v)
  ensures vb.v == val
  ensures Chosen(c, v.Last(), vb)  // modified from Chosen(c, v, vb)
  decreases c, v, lnr
{
  ...
  decreases c, v
{
  if !Safety(c, v) {
    ...
    ghost var vb1 := LearnedImpliesChosen(c, v, l1, v.Last().learners[l1].learned.value);  // modified from v.Last()
    ghost var vb2 := LearnedImpliesChosen(c, v, l2, v.Last().learners[l2].learned.value);  // modified from v.Last()
    assert false;
  }
}
...
lemma SupportingAcceptorsForChosen(c: Constants, v: Variables, vb: ValBal)
  ...
  requires v.WF(c)
  requires Chosen(c, v.Last(), vb)  // modified from Chosen(c, v, vb)
  requires LearnerReceivedAcceptImpliesAccepted(c, v)
...
ghost predicate OneValuePerBallotLeaderAndLearners(c: Constants, v: Variables)
  ...
  decreases c, v
{
  forall ldr: int, lnr: int, acceptedVal: Value
 {:trigger v.Last().learners[lnr], VB(acceptedVal, ldr)} {:trigger VB(acceptedVal, ldr), c.ValidLearnerIdx(lnr)}
 | c.ValidLeaderIdx(ldr) && c.ValidLearnerIdx(lnr) && VB(acceptedVal, ldr) in v.Last().learners[lnr].receivedAccepts.m :: 
    acceptedVal == v.Last().leaders[ldr].Value()
}
...
ghost predicate AtMostOneChosenVal(c: Constants, v: Variables)
  ...
  decreases c, v
{
  forall vb1: ValBal, vb2: ValBal
 {:trigger vb2.v, vb1.v} {:trigger vb2.v, vb1.b} {:trigger vb2.v, Chosen(c, v.Last(), vb1)} {:trigger vb1.v, vb2.b} {:trigger vb1.v, Chosen(c, v.Last(), vb2)} {:trigger vb2.b, vb1.b} {:trigger vb2.b, Chosen(c, v.Last(), vb1)} {:trigger vb1.b, Chosen(c, v.Last(), vb2)} {:trigger Chosen(c, v.Last(), vb2), Chosen(c, v.Last(), vb1)}
 | Chosen(c, v.Last(), vb1) && Chosen(c, v.Last(), vb2) :: 
