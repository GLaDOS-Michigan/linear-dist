...
// TODO this becomes a type parameter, perhaps includes a ghosty invariant promise.
...
ExactlyOnceDelivery {
    fields {
        ...
        #[sharding(map)]
        pub current_epoch: Map<HostId, nat>

        // And then there's a shared-knowledge set of
        // sent messages. How do we import the assumption
        // that ordinary-network-receive only delivers
        // sent messages? Is the set something we can
        // logically lock-read-unlock on every transition?
    }
    ...
    transition!{
        deposit_user_send(value: StoredType, destination: HostId) {
            deposit storage += Some(value);
        }
    }

    transition!{
        network_send(value: StoredType, destination: HostId) {
            birds_eye let value = pre.storage.get_Some_0();
            epoch += 1
            add network += Some(Message{value, epoch});
        }
    }

    transition!{
        network_receive(value: StoredType, me: HostId) {
            remove epoch -= [ me => let my_epoch ];
            have network =< Some(Message{value, msg_epoch});
            require my_epoch < msg_epoch;
            add epoch += [me => msg_epoch];
        }
    }

    transition!{
        withdraw_user_receive() {
